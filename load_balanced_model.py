...
class NGAN(pl.LightningModule):
    def __init__(
        ...
        self.balancer = Balancer(weights={'1': 1, '2': 1, '3':1, '4':1}, rescale_grads=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits_fake, fmap_fake = self.msstft_discriminator(output)
        ...
        loss_3 = l_t
        loss_4 = g_rec_loss
        losses = {'1': loss_1, '2': loss_2, '3': loss_3, '4': loss_4}
        input_orig.requires_grad_()
        g_loss = loss_1 + loss_2 + loss_3 + loss_4

        self.log("g_loss", g_loss, prog_bar=True)
        self.balancer.backward(losses, output)
    ...
    def _make_dataloader(self, train: bool):
        ...
        class VoiceDataset(torch.utils.data.Dataset):
            ...
            def __init__(self, root: str = self.hparams['train_data'], segment_length: int = self.hparams['segment_length'], target_sample_rate: int = self.hparams['target_sample_rate'] ) -> None:
                #siuper().__init__(root, download)
                
                self._flist = _get_flist(root)
                print(len(self._flist))
                self._segment_length = segment_length               

                       
            def __getitem__(self, n: int):
                x, sr = torchaudio.load(self._flist[n])
                resample_transform_x = T.Resample(orig_freq=sr, new_freq=48000)
                if sr > 48000:
                ...
                cutoff, fs, order = freq*0.5, 48000, 8
                z = lowpass_filter(x, cutoff, fs, order, ftype=random.choice(filters))
                z = librosa.resample(z, orig_sr=48000, target_sr=freq)
             
                v = torch.tensor(z.copy(), dtype=torch.float32)
                v_ = cubic_upsampler(v, freq, 48000)                
