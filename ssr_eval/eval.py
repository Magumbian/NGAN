...
class BasicTestee:
    ...
    def tensor2numpy(self, tensor):
        if "cuda" in str(tensor.device):
            ...
            #return tensor.detach().device().numpy
        else:
...
class SSR_Eval_Helper:
    ...
    def evaluate_single(self, file):
        ...
        target, sr = librosa.load(file, sr=48000, res_type="polyphase")
        ...
        for k in processed_low_res_input.keys():
            ...
            if self.model_output_sr != self.evaluationset_sr:
                processed = librosa.resample(
                    processed,
                    orig_sr = self.model_output_sr,
                    target_sr = self.evaluationset_sr,
                    res_type="polyphase",
                )
