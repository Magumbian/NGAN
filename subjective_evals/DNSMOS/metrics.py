...
from ssr_eval.plc_mos import PLCMOSEstimator
...
class AudioMetrics:
    ...
    def evaluation(self, est, target, file):
        ...
        result["mos"] = self.mos(est)
        result["mos2"] = self.mos2(est)
    ...
    def mos(self, est):
        ...
        mos = plcmos.run(est_16k, 16000)
        return(mos)

    def mos2(self, est):
        predictor = torch.hub.load("tarepan/SpeechMOS:v1.2.0", "utmos22_strong", trust_repo=True)
        score = predictor(torch.from_numpy(est).unsqueeze(0), 48000)
        return score
