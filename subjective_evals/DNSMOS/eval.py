...
class SSR_Eval_Helper:
    ...
    def pad_wav(self, waveform, target_length):
        ...
        assert waveform_length > 100, "Wavform length is too short, %s" % waveform_length
        temp_wav = np.zeros((target_length), dtype=np.float32)
        if(waveform.shape[0] <= target_length):
            temp_wav[0 : waveform_length] = waveform[:waveform_length]
        else:
            temp_wav[0 : target_length] = waveform[:target_length]
        return temp_wav
    ...
    def evaluate_single(self, file):
        ...
        #os.system("sox %s -r %s %s" % (file, self.evaluationset_sr, file))
        target, _ = librosa.load(file, sr=48000)
        
        """
        offset = duration % 5.12
        if(offset != 0):
            pad_duration = duration + 5.12 - offset
        else:
            pad_duration = duration
        """
        pad_duration = 5.12
        target_frame = int(pad_duration * 100)
        target_l = int(48000 * pad_duration)
        target = np.squeeze(self.pad_wav(target, target_l))
        print(target.shape)
        


        for k in processed_low_res_input.keys():
            print(processed_low_res_input[k].shape)
            result_fname = file + k + "_processed_" + self.test_name + ".flac"
            start = time.time()
            ...
            metrics[k]["inference_time"] = end - start
